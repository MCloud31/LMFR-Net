...
class ConfusionMatrix(object):
    ...
    def compute(self):
        ...
        # recall = TP / (TP + FN)
        recall = torch.diag(h)[1] / (torch.diag(h)[1] + h[0][1])
        # recall = h[0][1] / (h[0][1] + torch.diag(h)[1])
        # precision = TP / (TP + FP)
        precision = torch.diag(h)[1] / (torch.diag(h)[1] + h[1][0])
        f1 = (2 * precision * recall) / (precision + recall)
        # f1 = (2 * torch.diag(h)[1]) / (2 * torch.diag(h)[1] + h[0][1] + h[1][0])
        # specificity = TN / (TN + FP)
        sp = torch.diag(h)[0] / (torch.diag(h)[0] + h[1][0])
        # AUC = 1 - 1/2(FP/FP+TN + FN/FN+TP)
        auc = 1 - 0.5 * ((h[1][0] / (h[1][0] + torch.diag(h)[0])) + (h[0][1] / (h[0][1] + torch.diag(h)[1])))

        return acc_global, acc, iu, recall, f1, precision, sp, auc
    ...
    def __str__(self):
        acc_global, acc, iu, recall, f1, precision, sp, auc = self.compute()
        return (
            'global correct: {:.2f}\n'
            'average row correct: {}\n'
            'IoU: {}\n'
