...
#        self.res = nn.Conv2d(in_channels, out_channels, kernel_size=(1, 1), padding=0, bias=False)

#    def forward(self, x):
#        out = self.active(self.res(x) + self.layer(x))
#        return out
...
class SE_Block(nn.Module):
    ...
    def forward(self, X_input):
        ...
        y = y.view(b, c, 1, 1)  # shape = [16, 576, 1, 1]， 这个就表示上面公式的s, 即每个通道的权重
...
class LMFR_Net_B_ICB_LMAFF(nn.Module):
    def __init__(self,
                 base_c = 16,
                 num_classes = 2):
        super(LMFR_Net_B_ICB_LMAFF, self).__init__()
        # encoder
