...
from my_dataset_chasedb import ChaseDataset
...
def main(args):
    ...
    mean = (0.628, 0.224, 0.037)
    std = (0.221, 0.110, 0.036)

    results_file = "results.txt"
    
    train_dataset = ChaseDataset(args.data_path,
                                  train=True,
                                  transforms=get_transform(train=True, mean=mean, std=std))

    val_dataset = ChaseDataset(args.data_path,
                               train=False,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.save_best is True:
            torch.save(save_file, "save_weights/B_ICB_LMAFF_CHASEDB.pth")
        else:
            torch.save(save_file, "save_weights/B_ICB_LMAFF_CHASEDB-{}.pth".format(epoch))
